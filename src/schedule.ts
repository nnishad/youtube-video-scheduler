import { Credentials, Video, VideoProgress, ProgressEnum, MessageTransport, GameData } from './types'
import puppeteer from 'puppeteer-extra'
import { PuppeteerNodeLaunchOptions, Browser, Page, ElementHandle as PuppeteerElementHandle } from 'puppeteer'
import fs from 'fs-extra'
import path from 'path'
import StealthPlugin from 'puppeteer-extra-plugin-stealth'


const stealthPluginObj = StealthPlugin()
stealthPluginObj.enabledEvasions.delete('iframe.contentWindow')
stealthPluginObj.enabledEvasions.delete('navigator.plugins')
puppeteer.use(stealthPluginObj)
...
async function uploadVideo(videoJSON: Video, messageTransport: MessageTransport) {
    ...
    if ((await page.waitForXPath(createBtnXPath, { timeout: 5000 }).catch(() => null))) {
        ...
        if (createBtn.length > 0) {
            const elementHandle = createBtn[0];
            await (elementHandle as PuppeteerElementHandle).click();
            console.log('Clicked on the createBtn element!');
        } else {
    ...
    if (selectBtn.length > 0) {
        const [fileChooser] = await Promise.all([
            ...
            (selectBtn[0] as PuppeteerElementHandle).click() // button that triggers file selection
        ])
        await fileChooser.accept([pathToFile])

        console.log('Clicked on the selectBtn element!');
    } else {
        console.error('selectBtn not found!');
    }
    ...
    if (videoJSON.onProgress) {
        ...
        progressChecker = setInterval(async () => {
            let curProgress = await page.evaluate(() => {
                const items = document.querySelectorAll("span.progress-label.ytcp-video-upload-progress");
    ...
    // const uploadCompletePromise = page.waitForXPath('//tp-yt-paper-progress[contains(@class,"ytcp-video-upload-progress-hover") and @value="100"]',
    //     { timeout: 0 }).then(() => 'uploadComplete')
    //
    // // Check if daily upload limit is reached
    // const dailyUploadPromise = page.waitForXPath('//div[contains(text(),"Daily upload limit reached")]',
    //     { timeout: 0 }).then(() => 'dailyUploadReached');
    ...
    if (childOption.length > 0) {
        const elementHandle = childOption[0];
        await (elementHandle as PuppeteerElementHandle).click();
        console.log('Clicked on the childOption element!');
    } else {
        console.error('childOption not found!');
    }
    ...
    if (playlistName) {
        ...
        for (let i = 0; i < 2; i++) {
            try {
                ...
                await (playlist[0] as PuppeteerElementHandle).click()
                ...
                await (playlistNameSelector[0] as PuppeteerElementHandle).click()
                ...
                await (createplaylist[0] as PuppeteerElementHandle).click()
                ...
                await (createplaylistbtn[1] as PuppeteerElementHandle).click()
                // await page.evaluate((el) => (el as HTMLElement).click(), createplaylistbtn[1])
                createplaylistdone = await page.$x("//*[normalize-space(text())='Done']")
                await (createplaylistdone[0] as PuppeteerElementHandle).click()
                // await page.evaluate((el) => el.click(), createplaylistdone[0])
            }
        }
    }
...
async function loadAccount(credentials: Credentials, messageTransport: MessageTransport, useCookieStore = true) {
    try {
...
async function changeHomePageLangIfNeeded(localPage: Page) {
    ...
    const selectedLang = await localPage.evaluate(
        (langMenuItemSelector) => {
            const element = document.querySelector(langMenuItemSelector);
...
async function launchBrowser(puppeteerLaunch?: PuppeteerNodeLaunchOptions, loadCookies = true) {
...
async function login(localPage: Page, credentials: Credentials, messageTransport: MessageTransport, useCookieStore = true) {
    await localPage.goto(uploadURL)
...
async function changeChannel(channelName: string) {
    ...
    await page.waitForXPath(channelNameXPath);
    const element = await page.$x(channelNameXPath);
    if (element.length > 0) {
        const elementButton: PuppeteerElementHandle<HTMLDivElement> = element[0] as PuppeteerElementHandle<HTMLDivElement>;
        await elementButton.click();
        console.log('Clicked on the changeChannel!');
    } else {
        console.error('changeChannel button not found!');
    }
...
async function selectGame( page: Page, gameTitle: string, gameSelector?: ( arg0: GameData ) => Promise<boolean> | null ) {
    ...
    for ( let i = 0; i < buttonOptions.length; i++ ) {
        ...
        const testId = await button.evaluate( ( el: Element ) => el.getAttribute( "test-id" ) )
        if ( testId == null || !testId.startsWith( `{"title"` ) )
            continue

        // Parse the JSON.
        // console.log( `Game option: ${gameData.title}, ${gameData.year}` )
        const gameData = JSON.parse( testId ) as GameData
